...
def eval_exec_match(db, p_str, g_str, pred, gold, question, db_id, hardness, idx):
    """
    return 1 if the values between prediction and gold are matching
    in the corresponding index. Currently not support multiple col_unit(pairs).
    """
    # print("excution.......")
    # print(db, p_str, g_str)
    # conn = sqlite3.connect(db)
    # cursor = conn.cursor()
    # try:
    ...
    # # cursor.execute(g_str)
    # # q_res = cursor.fetchall()
    q_res = execution(db, g_str)
    if p_res:
        # print(p_res)
        p_res1 = ['\t'.join(sorted([str(x).lower() for x in _])) for _ in p_res]
        p_res2 = [str(_[0]).lower() for _ in p_res]
        # print(p_res1)
    ...
    if q_res:
        # print(q_res)
        q_res2 = [str(_[0]).lower() for _ in q_res]
        q_res1 = ['\t'.join(sorted([str(x).lower() for x in _])) for _ in q_res]
        # print(q_res1)
    else:
        q_res1 = []
        q_res2 = []
    
    if not ((set(frozenset(str(item)) for item in p_res1) == set(frozenset(str(item)) for item in q_res1) ) or (set(frozenset(str(item)) for item in p_res2) == set(frozenset(str(item)) for item in q_res2) )):
        # print(p_res)
        # print(q_res)
        # print(question)
        # print(p_str)
        ...
        logs = {
            ...
            'question': question,
            'db': db_id,
            'schema': "".join(Text2SQLCallLLM.get_schema_code(db_id)),
            'groundtruth_sql': g_str,
            'gpt_sql': p_str,
            'groundtruth_result': q_res,
            'gpt_result': p_res
        }
    ...
    # # print(res_map(q_res, q_val_units))
